...
async function emergency(req, res) {
  ...
  if (doctors.length != 0) {
    for (let i = 0; i < doctors.length; i++) {
      ...
      if (helperObj.length == 0) {
        ...
        let userObj = await userModel.findOne({ _id: emergencyObj._id })
        res.send({ message: `Successfully assigned with ${doctors[i].username}`, succ: "success", doctorName: doctors[i].username, hospitalName: doctors[i].hospitalName, emergency: emeObj, user: userObj });
        return;
        // }
  ...
  let doctorsww = await doctorModel.aggregate([
    {
      $match: {
    ...
    for (let i = 0; i < doctorsww.length; i++) {
      ...
      if (helperObj.length == 0) {
        ...
        if (s.length == 0) {
          ...
          let userObj = await userModel.findOne({ _id: emergencyObj._id })
          res.send({ message: `Successfully assigned with ${doctors[i].username} general physician`, succ: "success", doctorName: doctors[i].username, hospitalName: doctors[i].hospitalName, emergency: emeObj, user: userObj });
          return;
        }
      }
    }
