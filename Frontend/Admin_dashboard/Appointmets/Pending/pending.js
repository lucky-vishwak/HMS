...
function notAssignOnLoadAndOnClick() {
  for (let i = 0; i < appointmentsNotAssigned.length; i++) {
    ...
    let td_doctor = $("<td></td>").text(`${appointmentsNotAssigned[i].doctor.username}`);
  ...
  $.post({
    ...
    .done((response, stat) => {
      if (stat == 'success') {
        ...
        console.log('ghjk');
        console.log(appointmentsNotAssigned);
        $("#tbody").html('');
...
function assignedAppointmentsDisplay() {
  for (let i = 0; i < appointmentsAssigned.length; i++) {
    ...
    let td_doctor = $("<td></td>").text(`${appointmentsAssigned[i].doctor.username}`);
